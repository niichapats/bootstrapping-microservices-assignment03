...
async function main() {
    ...
    //
    // Gets the collection for storing viewed videos (modified for example-5).
    //
    ...
    await messageChannel.consume(queue, async (msg)=> {
        ...
        const parsedMsg = JSON.parse(msg.content.toString()); // { id, videoPath }

        const viewedValue = parsedMsg.id === "1" ? "viewed-1" : "viewed-2";
        ...
        console.log(`Inserted ${viewedValue} into videos collection`);
